...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def list_models() -> ModelListResponse:
        local = store.list_local_models()
        if settings.model_id not in local:
            local.insert(0, settings.model_id)
        data = [ModelInfo(id=m) for m in dict.fromkeys(local)]
        return ModelListResponse(data=data)

    async def stream_chat(data: dict[str, object]) -> AsyncGenerator[bytes, None]:
        chunk = {
            "id": data["id"],
    ...
    async def stream_completion(data: dict[str, object]) -> AsyncGenerator[bytes, None]:
        chunk = {
            "id": data["id"],
    ...
    async def audio_speech(req: AudioSpeechRequest):
        try:
            wav_bytes = runner.synthesize_speech(req.input, req.model)
        except RuntimeError as exc:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
        return Response(content=wav_bytes, media_type="audio/wav")

    @app.post("/v1/audio/transcriptions", dependencies=[Depends(check_auth)])
    async def audio_transcriptions(req: AudioTranscriptionRequest):
        raise HTTPException(
    ...
    async def api_pull(req: PullRequest):
        local_path = store.pull(req.model)
    ...
    async def api_delete(req: DeleteRequest):
        deleted = store.remove(req.model)
        if not deleted:
